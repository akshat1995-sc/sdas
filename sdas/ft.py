...
class FTArticleExtractor(ArticleExtractor):
    ...
    def _get_article_readable_url(self) -> str:
        ...
        if not main_html_content:
            raise ValueError('No div class "TEXT-BLOCK" in archive is url!')
    ...
    def _get_article_text(self) -> str:
        ...
        if not article_body:
            raise ValueError("No article body in archive url!")
        text = "".join(
            [div_element.text for div_element in article_body.find_all("div")]
        )
        return text
...
def get_contents_date_time(soup: BeautifulSoup) -> list[tuple[str, str]]:
    ...
    if not contents:
        raise ValueError("No ft article url in the soup!")
    date_time_strings = [
        date_time_content.time["datetime"] for date_time_content in date_time_contents
    ]
    return [
        (BASE_URL + content.a["href"], date_time_string)
        for content, date_time_string in zip(contents, date_time_strings)
    ]
